...
int lestringARQ(FILE *, char[], int);

/* Função para realizar a leitura de livros */
int lelivros(FILE *, struct LIVROS[], int, int);
...
int main(int argc, char *argv[])
{
    ...
    if (argc >= 2) {
        ...
        if (arqr == NULL) {
            printf("ERRO: %s\n", argv[1]);
            printf("# NAO Pude abrir %s para leitura\n", argv[1]);
            arqr = stdin;
        }
    }
    if (argc > 2) {
        ...
        if (arqw == NULL) {
            printf("ERRO: %s\n", argv[2]);
            printf("# NAO Pude abrir %s para escrita\n", argv[2]);
            arqw = stdout;
        }
    }
    if (arqr == stdin) {
        printf("# INFO: lendo do teclado\n");
    } else {
        printf("# INFO: aberto com sucesso %s\n", argv[1]);
    }
    if (arqw == stdout) {
        printf("# INFO: imprimindo na tela\n");
    } else {
        printf("# INFO: aberto com sucesso %s\n", argv[2]);
    }
    ...
    calcPg(NULL, &custo);
    for (i = 0; i < ql; i++) {
        fprintf(arqw, "  %-*s %-*s %*d R$ %*.2lf\n", tnomel, l[i].nomel, tautor, l[i].autor, tpag,
                l[i].pg, tvalor - 3, l[i].valor);
        totvalor += l[i].valor;
        totpg = totpg + l[i].pg;
        calcPg(&l[i], &custo);
    }
    ...
    fprintf(arqw, "  Total de paginas: %d\n", totpg);
    fprintf(arqw, "  Custo por pagina: R$ %.2lf\n", custo);
    printf("\n# Custo por pagina calcudo no main: R$ %.2lf\n", (double) totvalor / totpg);

    free(l);

    if (arqw != stdout) {
        printf("# Relatorio dos livros esta em %s\n", argv[2]);
        fclose(arqw);
    }
    if (arqr != stdin) {
        fclose(arqr);
...
int lelivros(FILE *onde, struct LIVROS l[], int ql, int maxstr)
{
    int i;
    ...
    for (i = 0; i < ql; i++) {
        ...
        lestringARQ(onde, l[i].nomel, maxstr);

        printf("\n# Nome do autor:\n");
        lestringARQ(onde, l[i].autor, maxstr);

        l[i].pg = -1;
        while (l[i].pg < 0) {
            printf("\n# Quantas páginas:\n");
            if (fscanf(onde, "%d", &l[i].pg) == 0) {
...
int lestringARQ(FILE *onde, char s[], int max)
{
    int i = 0;
    char letra;
